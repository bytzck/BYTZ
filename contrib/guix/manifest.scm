...
(define-public ncurses
  (package
    ...
    (source (origin
          ...
          (arguments '(#:tests? #f)
                     '(#:configure-flags `("--without-cxx" "--with-termlib" "--enable-cxx-shared" "--enable-symlinks" "--with-shared" "--without-tests"))))
  ...
  (let ((target (getenv "HOST")))
    (cond ((string-suffix? "-mingw32" target)
          ...
          ((string-contains target "darwin")
           (list clang-toolchain-10 libcap ncurses binutils imagemagick libtiff librsvg font-tuffy cmake xorriso python-signapple))
          (else '())))))
